...
def getFile(p:Path)->Iterable[Path]: #获取p路径下所有文件路径
    return [x for x in p.iterdir() if not x.is_dir()]

def getFolder(p:Path)->Iterable[Path]:
    return [x for x in p.iterdir() if x.is_dir()]

def getLang(flds:list)->list:
    return [x.stem for x in flds]

def getEvents(data_dir:Path, lang:str)->Iterable[Event]:
    with open(data_dir/lang/reviewPath, encoding='utf-8') as reviewFile:
...
def getRecords(data_dir:Path, lang:str)->Iterable[Event]:
...
def getMainline(data_dir:Path, lang:str)->Iterable[Event]:
    events = getEvents(data_dir, lang)
    records = []
    for event in events:
...
def getAct(data_dir:Path, lang:str)->Iterable[Event]:
    events = getEvents(data_dir, lang)
    records = []
    for event in events:
...
def getExtraAvg(data_dir:Path, lang:str)->Iterable[ExtraStory]:
